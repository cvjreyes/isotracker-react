...
export default class UploadInstrumentationEstimatedPopUp extends Component {
    constructor(props) {
        super(props);
        this.state = {
            visible : false,
    ...
    uploadReport(event){
        ...
        if(this.state.file.name.substring(this.state.file.name.length-4, this.state.file.name.length) === "xlsx"){
            readXlsxFile(this.state.file).then(async(rows) => {
                ...
                await fetch("http://"+process.env.REACT_APP_SERVER+":"+process.env.REACT_APP_NODE_PORT+"/uploadInstEstimatedReport", options)
    ...
    render() {
        return (
            <section style={{float:"left"}}>
                <input type="button"  value="Upload einsts" style={{marginTop:"10px",marginLeft:"10px", height:"150px", width:"150px"}} className="btn btn-bg btn-info"  onClick={() => this.openModal()} />
                <div>
                    <Modal visible={this.state.visible} width="650" height="180" effect="fadeInUp" onClickAway={() => this.closeModal()}>
                        <div className="popUp__container" >
                            <center className="popUp__title"><h2>Upload your estimated intrumentation report</h2></center>
