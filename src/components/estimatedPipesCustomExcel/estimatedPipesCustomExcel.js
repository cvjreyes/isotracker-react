...
class EstimatedPipesCustomExcel extends React.Component{
  state = {
    searchText: '',
    searchedColumn: '',
    data: [],
  ...
  await fetch("http://"+process.env.REACT_APP_SERVER+":"+process.env.REACT_APP_NODE_PORT+"/modelledEstimatedCustomPipes", options)
  .then(response => response.json())
  .then(async json => {
    let rows = [] 
    let tags = []
...
async componentDidUpdate(prevProps, prevState){
  ...
  async submitChanges(){
    ...
    await fetch("http://"+process.env.REACT_APP_SERVER+":"+process.env.REACT_APP_NODE_PORT+"/submitModelledEstimatedCustomPipes", options)
    .then(response => response.json())
    .then(async json => {
      if(json.success){
        this.props.success()
    ...
    if (source !== 'loadData'){
      ...
      for(let i = 0; i < changes.length; i+=4){
        if(changes[i][1] === "Owner IFC"){
          ...
          let row_id = changes[i][0]
          if(this.state.data[row_id].Status === "MODELLED*" || this.state.data[row_id].Status === "MODELLED" || this.state.data[row_id].Status === "MODELLED(F)" || this.state.data[row_id].Status === "ESTIMATED(F)"){
  ...
  render() {
    ...
    const settings = {
        ...
        colWidths: [180, 450, 240, 70, 70, 70, 100, 95, 80, 105, 110, 70, 160],
        fontSize: 24
        //... other options
      }
      ...
      if(this.props.role === "SpecialityLead"){
        col = [{ data: "Line reference", type:'dropdown', source: this.state.line_refs, strict: true}, { data: "Tag", type:'text', readOnly: true}, { data: "Owner IsoTracker", type:'dropdown', source: this.state.designers, strict: true}, { data: "Unit", type:'text', readOnly: true},{ data: "Area", type:'dropdown', source: this.state.areas, strict: true }, { data: "Fluid", type:'text', readOnly: true}, { data: "Seq", type:'text', readOnly: true}, { data: "Spec", type:'text', readOnly: true},  { data: "Type", type:'text', readOnly: true}, { data: "Diameter", type:'dropdown', source: this.state.diameters, strict: true}, { data: "Insulation", type:'text', readOnly: true},{ data: "Train", type:'dropdown', source: this.state.trains, strict: true},{ data: "Status", type:'dropdown', source: ["ESTIMATED(F)","MODELLED(F)"]}]
      }else{
...
export default EstimatedPipesCustomExcel;