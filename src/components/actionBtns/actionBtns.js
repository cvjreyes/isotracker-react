...
const ActionBtns = props =>{
    ...
    if (props.currentTab !== "My Tray" && props.currentTab !== "Recycle bin" && props.currentTab !== "On hold"){
        ...
        if(props.role !== "Design" && props.role !== "DesignLead"){
            actionBtn2 = <button class="btn btn-sm btn-danger" style={{marginRight:"5px", marginLeft:"5px"}} onClick={() => props.transaction("Design")}>Design</button>
        }
        if(process.env.REACT_APP_IFC === "0"){
            actionBtn3 = <button class="btn btn-sm btn-primary" style={{marginRight:"5px", marginLeft:"5px"}} onClick={() => props.transaction("stress")}>Stress</button>
            actionBtn4 = <button class="btn btn-sm btn-info" style={{marginRight:"5px", marginLeft:"5px"}} onClick={() => props.transaction("supports")}>Support</button>
            if(props.role !== "DesignLead" && props.role !== "StressLead" && props.role !== "SupportsLead"){
                actionBtn5= <button className="btn btn-sm btn-success" name="destination" value="stress" style={{marginRight:"5px", marginLeft:"5px"}} onClick={() => props.verifyClick()}>Verify</button>
            ...
            if(props.role !== "Design" && props.role !== "DesignLead"){
                ...
                actionBtn3 = <button class="btn btn-sm btn-info" style={{marginRight:"5px", marginLeft:"5px"}} onClick={() => props.transaction("supports")}>Supports</button>
            }else if(props.role === "SupportsLead"){
                actionBtn3 = <button class="btn btn-sm btn-primary" style={{marginRight:"5px", marginLeft:"5px"}} onClick={() => props.transaction("stress")}>Stress</button>
            }else{
                actionBtn3 = <button className="btn btn-sm btn-success" name="destination" value="stress" style={{marginRight:"5px", marginLeft:"5px"}} onClick={() => props.verifyClick()}>Verify</button>
